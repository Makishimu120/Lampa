(function() {
  ...
  if (!window.rch || !window.rch['https://rc.bwa.to']) {
    Lampa.Utils.putScript(["https://rc.bwa.to/invc-rch.js"], function() {}, false, function() {
      if (!window.rch['https://rc.bwa.to'].startTypeInvoke)
        window.rch['https://rc.bwa.to'].typeInvoke('https://rc.bwa.to', function() {});
    }, true);
  }

  function rchInvoke(json, call) {
    if (window.hubConnection && window.hubConnection['https://rc.bwa.to'])
      window.hubConnection['https://rc.bwa.to'].stop();

    if (!window.hubConnection)
      window.hubConnection = {};

    window.hubConnection['https://rc.bwa.to'] = new signalR.HubConnectionBuilder().withUrl(json.ws).build();
    window.hubConnection['https://rc.bwa.to'].start().then(function() {
      window.rch['https://rc.bwa.to'].Registry(window.hubConnection['https://rc.bwa.to'], function() {
        call();
      });
    })["catch"](function(err) {
  ...
  function ApiHttp() {
    ...
    this.menu = function(success, error) {
      ...
      network.silent(this.account(Defined.localhost)+'&rchtype='+(window.rch['https://rc.bwa.to'] ? window.rch['https://rc.bwa.to'].type : ''), function(data) {
        if (data.channels) {
          menu = data.channels;
          success(menu);
        } else {
      ...
      var load = function load() {
        ...
        menu.forEach(function(m) {
          function loadThis() {
            var separator = m.playlist_url.indexOf('?') !== -1 ? '&' : '?';
            var url_query = add_url_query.indexOf('?') !== -1 || add_url_query.indexOf('&') !== -1 ? add_url_query.substring(1) : add_url_query;
            network.silent(_this.account(m.playlist_url + separator + url_query), function(json) {
              if (json.rch) {
                rchRun(json, function() {
                  loadThis();
                });
