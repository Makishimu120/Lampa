(function() {
  ...
  if (!window.rch || !window.rch['{localhost}']) {
    Lampa.Utils.putScript(["{localhost}/invc-rch.js"], function() {}, false, function() {
      if (!window.rch['{localhost}'].startTypeInvoke)
        window.rch['{localhost}'].typeInvoke('{localhost}', function() {});
    }, true);
  }

  function rchInvoke(json, call) {
    if (window.hubConnection && window.hubConnection['{localhost}'])
      window.hubConnection['{localhost}'].stop();

    if (!window.hubConnection)
      window.hubConnection = {};

    window.hubConnection['{localhost}'] = new signalR.HubConnectionBuilder().withUrl(json.ws).build();
    window.hubConnection['{localhost}'].start().then(function() {
      window.rch['{localhost}'].Registry(window.hubConnection['{localhost}'], function() {
        call();
      });
    })["catch"](function(err) {
  ...
  function ApiHttp() {
    ...
    this.menu = function(success, error) {
      ...
      network.silent(this.account(Defined.localhost)+'&rchtype='+(window.rch['{localhost}'] ? window.rch['{localhost}'].type : ''), function(data) {
        if (data.channels) {
          menu = data.channels;
          success(menu);
        } else {
